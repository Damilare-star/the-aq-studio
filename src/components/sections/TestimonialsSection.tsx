    ...
    <section id="testimonials" className="bg-[#050505] section-padding">
      <div className="container-wide">
        ...
        >
          ...
          <div
            className="absolute -top-4 sm:-top-6 left-0 text-[rgba(139,92,246,0.12)] select-none pointer-events-none"
            style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(6rem, 15vw, 10rem)', lineHeight: 1 }}
            aria-hidden="true"
          >{"}"}</div>

          <div className="relative grid grid-cols-1 lg:grid-cols-[1fr_280px] xl:grid-cols-[1fr_320px] gap-10 lg:gap-12 xl:gap-20 items-start">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-[rgba(139,92,246,0.12)] border border-[rgba(139,92,246,0.20)] flex items-center justify-center shrink-0">
                    <span className="text-[#8B5CF6] text-xs font-semibold">{current.author.charAt(0)}</span>
                  </div>
                  <div>
                    <p className="text-white text-sm font-medium">{current.author}</p>
                    <p className="text-[rgba(255,255,255,0.35)] text-xs tracking-wide mt-0.5">{current.title}, {current.company}</p>
                  </div>
                  <div className="pl-4 border-l border-[rgba(255,255,255,0.08)]">
                    <p className="text-[rgba(255,255,255,0.25)] text-[10px] tracking-[0.16em] uppercase">{current.industry}</p>
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
            ...
            >
              {testimonials.map((t, i) => (
                ...
                >
                  <p className={"text-xs font-medium transition-colors duration-300 " + (i === active ? 'text-white' : 'text-[rgba(255,255,255,0.38)]')}>{t.author}</p>
                  <p className="text-[9px] tracking-[0.14em] uppercase text-[rgba(255,255,255,0.22)] mt-0.5">{t.company}</p>
                </button>
              ))}
            </div>
          </div>
